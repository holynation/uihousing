	...
	*/
	...
	private function loadTable(string $model,$data,$totalRow,$start,$length,$removeId=true){
		...
		if (empty($data) || $data==false) {
			...
			return "<div class='alert alert-warning text-dark' style='margin:0 auto;'>NO RECORD FOUND</div>";
		}
	...
	private function buildHtmlAction($data,$actionArray){//the link must be specified for the action that is to be performed
		...
		foreach ($actionArray as $key => $value) {
			...
			if(method_exists($this->tableActionModel,$value)){
				...
				$tempLink = $tempObj['link'].'/'.$currentid;
				$link = $tempLink;
			...
			$result.="<span data-item-id='$currentid' data-default='$default' data-critical='$critical' $editClass><a href='$link' class='dropdown-item text-center text-capitalize'>$label</a></span>";
		}
		return $result;
	}
	...
	private function generateBody($data,$exclusionArray=array(),$actionArray=array(),$icon=''){
		...
		if (empty($data) || $data==false) {
			...
			return "<tbody><tr><td colspan='$countHeader'><div class='alert alert-warning text-dark text-center'>
			$icon<p>
			No ".
	...
	private function dropDownAction(){
